...
describe('health client', () => {
  it('accepts a same-origin API path', () => {
    expect(apiBaseUrl('/api', 'http://localhost:5173')).toBe(
  ...
  it('returns a validated health response', async () => {
    ...
    await expect(
      getHealth('http://localhost:5173/api', request),
  ...
  it('rejects invalid responses and request failures', async () => {
    ...
    await expect(getHealth('http://localhost/api', failed)).rejects.toThrow(
      'offline',
    )
  })
})
