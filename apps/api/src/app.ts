...
import { RecordNotFound, prismaSchoolStore, type SchoolStore } from './schoolService.js'
...
export function buildApp(options: { store?: SchoolStore } = {}) {
  ...
  app.setErrorHandler((error, _request, reply) => {
    if (error instanceof ZodError) {
      return reply.code(400).send(ErrorResponseSchema.parse({
    ...
    app.log.error(error)
    return reply.code(500).send(ErrorResponseSchema.parse({
